 ...
 *
 */
public class mwkim_20190114_02 {
	static int[] ticket, visitPlan;
	static int min_result;
	static int[] month = {1, 1, 3};

	static void calcPayPlan(int curMonth, int result) {
		//System.out.println("[" + curMonth + "]: " + visitPlan[curMonth] + " | " + result);		
		if(curMonth >= 12) {
			min_result = Math.min(min_result, result);
			return;
		}
		
		if(visitPlan[curMonth] == 0) {
			calcPayPlan(curMonth + 1, result);
			return;
		}
		
		for(int i = 0; i < 3; i++) {
			int new_month = curMonth + month[i] >= 13 ? 12 : curMonth + month[i];
			int new_result = (i == 0) ? result + (visitPlan[curMonth] * ticket[i]): result + ticket[i];

			calcPayPlan(new_month, new_result);
		}
	}
